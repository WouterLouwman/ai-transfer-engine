...
def tactical_fit(p, t):
    ...
    for k, w in sys_need.items():
        dist += w * abs(player_style[k] - float(t[k]))
        wsum += w
...
for _ in range(N_SCENARIOS):
    ...
    t = teams.sample(1, random_state=int(rng.integers(0, 1e9))).iloc[0]
    ...
    # --- NEW: Deal economics (so success changes with fee/wage/years) ---
    market_value = float(p.get("market_value_eur", 30_000_000))
    transfer_fee = market_value * float(rng.uniform(0.8, 2.0))
    weekly_wage = float(rng.uniform(20_000, 400_000))
    contract_years = int(rng.integers(2, 6))

    total_cost = transfer_fee + weekly_wage * 52 * contract_years
    cost_pressure = total_cost / max(market_value, 1.0)  # >2 = expensive deal

    # Hidden "true" success score -> used to create label
    score = (
        0.10 * (float(p["overall"]) - 75)
        + 1.2 * float(p["xg_p90"])
        + 1.0 * float(p["xa_p90"])
        + 0.25 * (float(p["tackles_p90"]) + float(p["interceptions_p90"])) / 2
        - 0.9 * float(p["injury_prone"])
        - 0.04 * (float(p["age"]) - 26) ** 2
        - 2.0 * (league_jump - 1.0) ** 2
        + 1.2 * fit
        # expensive deals have higher "pressure"/lower chance of success
        - 0.6 * (cost_pressure - 2.0)
    )
